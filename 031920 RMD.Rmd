...
Part II: Keep precursors that have 40-60% GC content, and have a melting temperature (Tm) of 50-60°C. (Emma) 
...
the “Tm Calculator” package. Once the Tm_NN is calculated for each primer, by row, the primers that do not have a Tm between 50°-60°C are filtered out, similar to the GC-content workflow.The "Biostrings" package is also necessary for calcualting the Tm, as the Tm Calculator package requires strings, rather than characters, which is converted by the c2s function found within this package.
  ...
  Additionally, it is necessary to determine how many rows are in each of the primer_precursor dataframes. Because each dataframe contains different numbers of primers that have overcome the GC threshold, it is necessary for the user to input the number of rows, which is found using the nrow function. Next, the user creates an empty vector, which will then be filled with the Tm values calculated in the for loop. The foor loop takes every row in the pp_24bp_pass_GC_threshold dataframe and runs the Tm_NN function, depositing the calculated Tm values into the pp_24bp_pass_GC_threshold_Tm vector.
```{r}
...
#define how many rows have made it through the GC threshold
...
#for loop that runs the Tm_NN function over each row in the pp_24bp_pass_GC_threshold dataframe
...
once both forward and reverse strands are designed.  
  ...
  Another current shortfall in our code for calculating the Tm of these primers, using the Tm_NN function, is in regards to the user inputting the number of rows present in the data frame 
