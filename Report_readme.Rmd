...
Now that we have the final primers, we just need format the final output, clean it to get rid of Ns and print it in a table
...
In the full R code,I used tibble from tidyverse to get the complement sequence based off the forward sequence (change A to T, etc.). Next,I generated a list and data frame of primers and complement primers that start and end with G/C pairs. Finally, my code unites and prints a final table of primer and complement primer that begin and end with G/C pairs along with the GC content and Tm. 
